...
2. Создать файл [.env]() и скопировать в него переменную `API_ACCESS_TOKEN `из файла [.env.sample](.env.sample);
3. Скопировать полученный API Access Key  в переменную `API_ACCESS_TOKEN` находящуюся в файле [.env]();
4. Запустить программу командой `python3 manage.py runserver`;
5. В эндпоинте `/converter/` заполнить 3 обязательных поля: 
